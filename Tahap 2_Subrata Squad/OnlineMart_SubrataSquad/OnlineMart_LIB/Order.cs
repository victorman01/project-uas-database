...
namespace OnlineMart_LIB
{
    public class Order
    {
        ...
        public static List<Order> BacaData(string kriteria, string nilaiKriteria)
        {
            ...
            if (kriteria == "")
            {
                sql = "select o.id, o.tanggal_waktu, o.alamat_tujuan,o.ongkos_kirim, o.total_bayar,o.cara_bayar,c.id,c.nama,c.alamat,p.id, p.nama,p.email,p.password,p.telepon, d.id,d.nama,d.email,d.password,d.telepon,pe.id,pe.nama,pe.email,pe.password,pe.telepon,pe.saldo,pe.poin,pr.id,pr.tipe,pr.nama,pr.diskon,pr.diskon_max,pr.minimal_belanja,o.status,mp.nama,o.status_kirim from orders o inner join cabangs c on o.cabangs_id = c.id inner join pegawais p on c.pegawais_id = p.id inner join drivers d on o.drivers_id = d.id inner join promos pr on o.promo_id = pr.id  inner join pelanggans pe on o.pelanggans_id = pe.id inner join metode_pembayarans mp on o.metode_pembayaran_id = mp.id;";
            }
            else
            {
                sql = "select o.id, o.tanggal_waktu, o.alamat_tujuan,o.ongkos_kirim, o.total_bayar,o.cara_bayar,c.id,c.nama,c.alamat,p.id, p.nama,p.email,p.password,p.telepon, d.id,d.nama,d.email,d.password,d.telepon,pe.id,pe.nama,pe.email,pe.password,pe.telepon,pe.saldo,pe.poin,pr.id,pr.tipe,pr.nama,pr.diskon,pr.diskon_max,pr.minimal_belanja,o.status,mp.nama,o.status_kirim from orders o inner join cabangs c on o.cabangs_id = c.id inner join pegawais p on c.pegawais_id = p.id inner join drivers d on o.drivers_id = d.id inner join promos pr on o.promo_id = pr.id  inner join pelanggans pe on o.pelanggans_id = pe.id inner join metode_pembayarans mp on o.metode_pembayaran_id = mp.id where " + kriteria + " like '%" + nilaiKriteria + "%'";
            }
